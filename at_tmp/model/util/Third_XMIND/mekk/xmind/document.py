...
import zipfile
from model.util.Third_XMIND.mekk.xmind.id_gen import IdGen, qualify_id, unique_id
from model.util.Third_XMIND.mekk.xmind.xmlutil import XmlHelper, ns_name, \
...
SHAPE_ROUND_RECTANGLE = "org.xmind.ui.logic.right"
...
class Sheet(DocumentPart):
    ...
    def create(cls, doc, sheet_name, root_topic_name):
        ...
        topic_tag = doc.create_child(sheet_tag, u"topic",
                                     id = _id_gen.next(),attrib = {
                             # "line-color" : "#CACACA",
                             "modified-by" : "Administrator",
                             "structure-class" : SHAPE_ROUND_RECTANGLE,
    ...
    def get_root_topic(self,*args, **kwargs):
        """
...
class Topic(DocumentPart):
    ...
    """
    TAG_NAME = ""
    def __init__(self, doc, topic_tag, node=None):
        DocumentPart.__init__(self, doc)
...
class XMindDocument(XmlHelper):
    ...
    def _add_to_zip(self, zipf, name, content):
        ...
        zipf.writestr(name, str(content))
